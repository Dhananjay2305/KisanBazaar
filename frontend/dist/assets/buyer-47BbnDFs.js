...
import{s as c,r as b}from"./api-DqiYm43F.js";import{a as x}from"./auth-DlKCV7tk.js";import{_ as B}from"./preload-helper-ckwbz45p.js";let v=null,m=[],p=null;function k(){if(!navigator.geolocation){c("Geolocation not supported","error");return}c("Detecting location...","info"),navigator.geolocation.getCurrentPosition(e=>{p={lat:e.coords.latitude,lng:e.coords.longitude},c("Location updated! Sorting by distance...","success"),document.getElementById("priceSort").value="distance",y()},e=>{c("Location error: "+e.message,"error")})}function C(e,t,o,r){if(!e||!t||!o||!r)return null;const i=6371,a=(o-e)*Math.PI/180,n=(r-t)*Math.PI/180,s=Math.sin(a/2)*Math.sin(a/2)+Math.cos(e*Math.PI/180)*Math.cos(o*Math.PI/180)*Math.sin(n/2)*Math.sin(n/2),u=2*Math.atan2(Math.sqrt(s),Math.sqrt(1-s));return i*u}document.addEventListener("DOMContentLoaded",async()=>{try{const e=await x("buyer");if(!e){localStorage.removeItem("token"),localStorage.removeItem("user"),window.location.replace("auth.html");return}if(document.getElementById("userName").textContent=e.name,document.getElementById("profileName").value=e.name,document.getElementById("profileEmail").value=e.email||"",document.getElementById("profilePhone").value=e.phone||"",document.getElementById("profileRole").value=e.role,document.getElementById("profileLocation").value=e.location||"",e.profile_image){const{resolveImageUrl:r}=await B(async()=>{const{resolveImageUrl:i}=await import("./api-DqiYm43F.js").then(a=>a.i);return{resolveImageUrl:i}},__vite__mapDeps([0,1]));document.getElementById("profileImagePreview").src=r(e.profile_image)}T(),y(),F(),I(),$();const t=["browse","cart","offers","orders","profile"],o=window.location.hash.replace("#","");t.includes(o)?f(o):f("browse")}catch(e){console.error("Buyer dashboard initialization failed:",e),c(e.message||"Connection offline. Retrying in background...","error")}finally{document.documentElement.classList.remove("auth-checking")}});document.querySelectorAll(".sidebar__link").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const o=t.currentTarget.dataset.section;f(o)})});window.addEventListener("hashchange",()=>{const e=["browse","cart","offers","orders","profile"],t=window.location.hash.replace("#","");e.includes(t)&&f(t)});function f(e){var o,r;(o=document.getElementById("sidebar"))==null||o.classList.remove("open"),["browse","cart","offers","orders","profile"].includes(e)&&(document.querySelectorAll(".sidebar__link").forEach(i=>i.classList.remove("active")),(r=document.querySelector(`[data-section="${e}"]`))==null||r.classList.add("active"),document.getElementById("browseSection").style.display=e==="browse"?"block":"none",document.getElementById("offersSection").style.display=e==="offers"?"block":"none",document.getElementById("ordersSection").style.display=e==="orders"?"block":"none",document.getElementById("cartSection").style.display=e==="cart"?"block":"none",document.getElementById("profileSection").style.display=e==="profile"?"block":"none",e==="cart"?E():e==="orders"&&N(),window.location.hash!==`#${e}`&&(window.location.hash=e))}function S(){const e=document.getElementById("sidebar"),t=document.getElementById("mobileOverlay");e.classList.toggle("open"),t&&(t.style.display=e.classList.contains("open")?"block":"none")}async function T(){try{const e=await api.get("/stats/buyer");document.getElementById("statTotalOffers").textContent=e.total_offers,document.getElementById("statAccepted").textContent=e.accepted_offers,document.getElementById("statPending").textContent=e.pending_offers,document.getElementById("statRejected").textContent=e.rejected_offers}catch(e){console.error("Stats error:",e.message)}}async function I(){try{const e=await api.get("/notifications"),t=document.getElementById("notifBadge"),o=document.getElementById("notifList");if(e.unread_count>0?(t.style.display="flex",t.textContent=e.unread_count>9?"9+":e.unread_count):t.style.display="none",e.notifications.length===0){o.innerHTML='<p style="padding:1rem;color:var(--gray-500);text-align:center;">No notifications</p>';return}o.innerHTML=e.notifications.slice(0,20).map(r=>`
                    ...
                    <div class="card">
                        ...
                        <div class="card__content">
                            ...
                            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem;">
                                <img src="${h((s=n.farmer)==null?void 0:s.profile_image,(u=n.farmer)==null?void 0:u.name)}" alt="Farmer" style="width: 24px; height: 24px; border-radius: 50%; object-fit: cover;">
                ...
                `}).join("")}catch(t){e.innerHTML=`<p style="color: var(--error);">Error loading listings: ${t.message}</p>`}}let d=JSON.parse(localStorage.getItem("cart")||"[]"),l=null;function $(){const e=document.getElementById("cartBadge");e&&(d.length>0?(e.style.display="inline-block",e.textContent=d.length):e.style.display="none")}function w(){localStorage.setItem("cart",JSON.stringify(d)),$()}function L(){if(!l)return;const t=(parseInt(document.getElementById("buyQuantity").value)||0)*l.price;document.getElementById("modalTotalPrice").textContent=`₹${t}`}async function F(){try{const t=(await api.get("/offers/sent")).offers||[],o=document.getElementById("offersTableBody"),r=document.getElementById("noOffersMessage"),i=document.getElementById("offersTable");if(t.length===0){i.style.display="none",r.style.display="block";return}i.style.display="table",r.style.display="none",o.innerHTML=t.map(a=>{var u,g,_;const n=((u=a.listing)==null?void 0:u.farmer)||{},s=n.phone||"";return`
                    <tr>
                        <td><strong>${((g=a.listing)==null?void 0:g.crop_name)||"N/A"}</strong></td>
                        <td>
                            <div style="display: flex; align-items: center; gap: 0.5rem;">
                                <img src="${h(n.profile_image,n.name)}" style="width: 24px; height: 24px; border-radius: 50%; object-fit: cover;">
                                <div>
                                    ${n.name||"Unknown"}<br>
                ...
                <div style="display: flex; gap: 1rem; align-items: center;">
                    ...
                    <div style="text-align: right; border-left: 1px solid var(--gray-200); padding-left: 1rem;">
                        <img src="${h((o=t.farmer)==null?void 0:o.profile_image,(r=t.farmer)==null?void 0:r.name)}" alt="Farmer" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover; margin-bottom: 0.25rem;"><br>
                        <small style="color: var(--gray-500);">${((i=t.farmer)==null?void 0:i.name)||"Farmer"}</small>
                    </div>
                </div>
            `,L(),document.getElementById("offerModal").classList.add("active"))}function M(){document.getElementById("offerModal").classList.remove("active"),document.getElementById("buyForm").reset(),l=null}document.getElementById("buyForm").addEventListener("submit",e=>{var i,a,n;if(e.preventDefault(),!l)return;const t=parseInt(document.getElementById("buyQuantity").value),o=document.getElementById("buyMessage").value,r=d.find(s=>s.listing_id===l.id);r?(r.quantity+=t,r.quantity>l.quantity&&(r.quantity=l.quantity)):d.push({listing_id:l.id,crop_name:l.crop_name,price:l.price,quantity:t,unit:l.unit,image:l.image,farmer_id:l.farmer_id||((i=l.farmer)==null?void 0:i.id),farmer_name:((a=l.farmer)==null?void 0:a.name)||"Farmer",farmer_image:((n=l.farmer)==null?void 0:n.profile_image)||"",message:o}),w(),c("Added to cart!","success"),M()});function E(){const e=document.getElementById("cartList"),t=document.getElementById("cartEmpty"),o=document.getElementById("cartTable"),r=document.getElementById("cartFooter");if(d.length===0){o.style.display="none",r.style.display="none",t.style.display="block";return}o.style.display="table",r.style.display="flex",t.style.display="none";let i=0;e.innerHTML=d.map((a,n)=>{const s=a.price*a.quantity;return i+=s,`
                    <tr>
                        <td>
                            <div style="display: flex; align-items: center; gap: 1rem;">
                                <img src="${b(a.image)}" style="width: 40px; height: 40px; border-radius: 0.25rem; object-fit: cover;">
                                <div>
                                    <strong>${a.crop_name}</strong><br>
                                    <div style="display: flex; align-items: center; gap: 0.25rem; margin-top: 0.25rem;">
                                        <img src="${h(a.farmer_image,a.farmer_name)}" style="width: 16px; height: 16px; border-radius: 50%; object-fit: cover;">
                ...
                `}).join(""),document.getElementById("cartTotalAmount").textContent=`₹${i}`}function U(e){d.splice(e,1),w(),E()}function j(){confirm("Are you sure you want to empty your cart?")&&(d=[],w(),E())}function H(){d.length!==0&&(window.location.href="checkout.html")}async function N(){try{const t=(await api.get("/orders/buyer")).orders||[],o=document.getElementById("ordersTableBody"),r=document.getElementById("noOrdersMessage"),i=document.getElementById("ordersTable");if(t.length===0){i.style.display="none",r.style.display="block";return}i.style.display="table",r.style.display="none",o.innerHTML=t.map(a=>`
                    <tr>
