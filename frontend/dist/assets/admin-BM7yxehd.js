...
import{s as o,a as i,f as u,b as p,r as h}from"./api-DqiYm43F.js";import{_ as P}from"./preload-helper-ckwbz45p.js";import{b as $}from"./auth-DlKCV7tk.js";import"./push-notifications-CQmv0i8S.js";window.toggleMobileSidebar=function(){document.querySelector(".admin-sidebar").classList.toggle("mobile-open"),document.getElementById("adminMobileOverlay").classList.toggle("active")};window.closeMobileSidebar=function(){document.querySelector(".admin-sidebar").classList.remove("mobile-open"),document.getElementById("adminMobileOverlay").classList.remove("active")};window.doLogout=async function(a){a.preventDefault(),localStorage.removeItem("token"),localStorage.removeItem("user");try{const{supabase:t}=await P(async()=>{const{supabase:e}=await import("./api-DqiYm43F.js").then(n=>n.e);return{supabase:e}},__vite__mapDeps([0,1]));await t.auth.signOut()}catch{}window.location.href="admin-login.html"};window.sendTestPushNotification=async function(){const a=document.getElementById("btnTestPush"),t=a.innerHTML;a.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Sending...',a.disabled=!0;try{const e=window.location.hostname.includes("localhost")?"http://localhost:5001/api/push/send":"/_/backend/api/push/send";(await fetch(e,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:"FarmDirect Admin Broadcast",body:"This is a test notification sent from the Admin Panel!"})})).ok?alert("Broadcast notification sent successfully!"):alert("Failed to send broadcast notification.")}catch(e){console.error(e),alert("Error sending notification: "+e.message)}finally{a.innerHTML=t,a.disabled=!1}};let s={},m=[];document.addEventListener("DOMContentLoaded",async()=>{var a;try{const t=await $();if(!t){localStorage.removeItem("token"),localStorage.removeItem("user"),window.location.href="admin-login.html";return}const e=document.getElementById("adminUserInfo"),n=document.getElementById("adminAvatar");e&&t.name&&(e.textContent=t.name),n&&t.name&&(n.textContent=t.name.charAt(0).toUpperCase()),(a=document.getElementById("adminProductImage"))==null||a.addEventListener("change",M);const d=["dashboard","orders","products","customers","payments","support","analytics"],r=window.location.hash.replace("#","");d.includes(r)?l(r):l("dashboard")}catch(t){console.error("Admin dashboard initialization failed:",t),o(t.message||"Connection offline. Retrying in background...","error")}window.addEventListener("hashchange",()=>{const t=window.location.hash.replace("#","");validViews.includes(t)&&l(t)}),window.switchView=l,window.deleteProduct=k,window.updateOrderStatus=x,window.openAddProductModal=F,window.openEditProductModal=D,window.closeModal=I,window.openAddFarmerModal=H,window.closeFarmerModal=C});function M(a){const t=a.target.files[0],e=document.getElementById("adminProductImagePreview"),n=document.getElementById("adminProductImageName");if(!e)return;if(!t){e.style.display="none",e.removeAttribute("src"),n&&(n.textContent="");return}if(!t.type.startsWith("image/")){o("Please choose a JPEG, PNG, or WebP image.","error"),a.target.value="";return}const d=new FileReader;d.onload=r=>{e.src=r.target.result,e.style.display="block"},d.readAsDataURL(t),n&&(n.textContent=t.name)}function v(a=""){const t=document.getElementById("adminProductImagePreview"),e=document.getElementById("adminProductImage"),n=document.getElementById("adminProductImageName");e&&(e.value=""),a?(t.src=h(a),t.style.display="block",n&&(n.textContent="Current image (choose a file to replace)")):(t.style.display="none",t.removeAttribute("src"),n&&(n.textContent=""))}function y(a,t="Save Product"){const e=document.getElementById("saveProductBtn");e&&(e.disabled=a,e.innerHTML=a?'<span class="loading-spinner"></span> Saving...':t)}async function l(a){var n,d;if((n=document.getElementById("adminSidebar"))==null||n.classList.remove("mobile-open"),(d=document.getElementById("adminOverlay"))==null||d.classList.remove("active"),!["dashboard","orders","products","customers","payments","support","analytics"].includes(a))return;document.querySelectorAll(".admin-nav__item").forEach(r=>{var c;r.classList.remove("active"),(c=r.getAttribute("onclick"))!=null&&c.includes(a)&&r.classList.add("active")});const e={dashboard:"Dashboard Overview",orders:"Orders Management",products:"Product Catalog",customers:"Customer Management",payments:"Payment Tracking",support:"Customer Support",analytics:"Detailed Analytics"};switch(document.getElementById("pageTitle").innerText=e[a]||"Admin Panel",document.querySelectorAll(".view-section").forEach(r=>r.classList.remove("active")),document.getElementById(`view-${a}`).classList.add("active"),a){case"dashboard":await T();break;case"orders":await w();break;case"products":await g();break;case"customers":await E();break;case"payments":await S();break;case"support":await A();break;case"analytics":await O();break}window.location.hash!==`#${a}`&&(window.location.hash=a)}async function T(){try{const a=await i.get("/admin/stats"),t=document.getElementById("dashboardStats");t.innerHTML=`
        ...
        `,L(a)}catch(a){o("Error loading dashboard: "+a.message,"error")}}function L(a){const t=document.getElementById("dashboardOrdersChart").getContext("2d"),e=document.getElementById("dashboardRevenueChart").getContext("2d");s.dashOrders&&s.dashOrders.destroy(),s.dashRevenue&&s.dashRevenue.destroy();const n=a.recentOrders.slice(0,7).reverse();s.dashOrders=new Chart(t,{type:"line",data:{labels:n.map(d=>new Date(d.created_at).toLocaleDateString()),datasets:[{label:"Order Value",data:n.map(d=>d.total_amount),borderColor:"#22c55e",tension:.4,fill:!0,backgroundColor:"rgba(34, 197, 94, 0.1)"}]},options:{responsive:!0,plugins:{legend:{display:!1}}}}),s.dashRevenue=new Chart(e,{type:"doughnut",data:{labels:["Farmers","Buyers"],datasets:[{data:[a.roleStats.farmer,a.roleStats.buyer],backgroundColor:["#22c55e","#3b82f6"]}]},options:{responsive:!0}})}async function w(){try{const{orders:a}=await i.get("/admin/orders"),t=document.getElementById("ordersTableBody");t.innerHTML=a.map(e=>{var n;return`
            <tr>
                <td>#${e.id.slice(-6)}</td>
        ...
        `}).join("")}catch(a){o("Error loading orders: "+a.message,"error")}}async function g(){try{const{products:a}=await i.get("/admin/products");m=a||[];const t=document.getElementById("productsTableBody");if(!m.length){t.innerHTML='<tr><td colspan="7" style="text-align:center;padding:2rem;color:#6b7280;">No products yet. Click "Add Product" to create one.</td></tr>';return}t.innerHTML=m.map(e=>{var n;return`
            <tr>
        ...
        `}).join("")}catch(a){o("Error loading products: "+a.message,"error")}}async function E(){try{const{customers:a}=await i.get("/admin/customers"),t=document.getElementById("customersTableBody");t.innerHTML=a.map(e=>`
            <tr>
        ...
        `).join("")}catch(a){o("Error loading customers: "+a.message,"error")}}async function S(){try{const{payments:a}=await i.get("/admin/payments"),t=document.getElementById("paymentsTableBody");t.innerHTML=a.map(e=>`
            <tr>
        ...
        `).join("")}catch(a){o("Error loading payments: "+a.message,"error")}}async function A(){const a=document.getElementById("supportTableBody");try{const{messages:t}=await i.get("/admin/support");if(!t||t.length===0){a.innerHTML=`<tr><td colspan="5" style="text-align:center;padding:3rem;color:#6b7280;">
